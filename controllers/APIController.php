...
public static function deleteEmployee() {
    if($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        error_log("Petición POST recibida en deleteEmployee");
        ...
        if($user) {
            error_log("Usuario encontrado: ID " . $id);

            $registros = Registro::findAllEmployesWhere('idempleado', $id);
            foreach ($registros as $registro) {
                error_log("Borrando registro ID: " . $registro->id);
                $registro->delete();
            }
            ...
            error_log("Usuario no encontrado: ID " . $id);
        }
